...
class Chess_Env:
    ...
    def __init__(self, N_grid, r_checkmate, r_draw):
        ...
        self.r_checkmate=r_checkmate           # reward of checkmate
        self.r_draw=r_draw                     # reward of draw
        
        self.num_of_steps = 0;                 # Number of steps spent by agents in each episode
        self.is_checkmate = 0;                 # Did the agent checkmate？（For counting the rate of checkmate later）
        ...
        self.check=0                                   # 1 (0) IF ENEMY KING (NOT) IN CHECK 1 (0) 
    ...
    def OneStep(self,a_agent):
        ...
        # Each time oneStep() is executed, the number of steps will be increased by one
        self.num_of_steps += 1;
        self.is_checkmate = 0;
        
        # If not done, default reward is 0.
        R= 0
        ...
        if np.sum(self.dfk2_constrain) == 0 and self.dfq1[self.p_k2[0], self.p_k2[1]] == 1:
            ...
            # ------------------------------------------------------------------------
            R = self.r_checkmate           # Reward for checkmate
            # ------------------------------------------------------------------------
            allowed_a=[]   # Allowed_a set to nothing (end of the episode)
            X=[]           # Features set to nothing (end of the episode)
            self.num_of_steps = 0; # If done, reset num_of_steps to 0
            self.is_checkmate = 1; # change the checkmate state --> is checkmate --> 1
        ...
        # CASE OF DRAW
            ...
            X=[]            # Features set to nothing (end of the episode)
            self.num_of_steps = 0; # If done, reset num_of_steps to 0
            self.is_checkmate = 0; # change the win state --> not checkmate --> 0
